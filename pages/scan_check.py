...
if st.button('計算を行う'):
    ...
    for code in codes1:
        ...
        source['EMA12'] = exp12
        source['EMA15'] = exp15
        exp25 = source['Close'].ewm(span=25, adjust=False).mean()
        source['EMA25'] = exp25
        ...
        exp200 = source['Close'].ewm(span=200, adjust=False).mean()
        source['EMA200'] = exp200
        ...
        window=25
        source['av'] = np.where(source['Close'].diff() > 0, source['Volume'], 0)
        avs = source['av'].rolling(window=window, center=False).sum()
        source['bv'] = np.where(source['Close'].diff() < 0, source['Volume'], 0)
        bvs = source['bv'].rolling(window=window, center=False).sum()
        source['cv'] = np.where(source['Close'].diff() == 0, source['Volume'], 0)
        cvs = source['cv'].rolling(window=window, center=False).sum()
        source.drop(['av', 'bv', 'cv'], inplace=True, axis=1)

        source['vr'] = (avs + cvs / 2) / (bvs + cvs / 2) * 100  
        source['distance25'] = (source['Close'] - source['sma02'])/source['sma02'] *100
